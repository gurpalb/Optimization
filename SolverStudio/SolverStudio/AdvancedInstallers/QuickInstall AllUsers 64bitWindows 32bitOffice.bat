...
set _batchFileName=C:\Program Files (x86)\SolverStudio\SolverStudio\AdvancedInstallers\QuickInstall AllUsers 64bitWindows 32bitOffice.bat
...
ECHO "%_batchFileName%"
ECHO.
ECHO Please copy the SolverStudio folder into C:\Program Files (x86), and try again.
ECHO.
pause
GOTO:EOF
...
set _vstoFile=C:\Program Files (x86)\SolverStudio\SolverStudio\SolverStudio.vsto
...
regedit "%~dp0SupportFiles\Install_HKLM_OS64_Office32.reg"
...
:SubSaveDir 
REM This saves the name of the current sub-directory , with \ appended
REM Using a subroutine like this is one way of forcing delayed expansion of arguments
set _src=%~dpn1\
GOTO:EOF
